...
Controller::Controller(const mc_rbdyn::RobotModulePtr &rm, const double &dt,
  ...
  logger().addLogEntry("C_res", [this]() {
    Eigen::Vector3d surfaceNormal =
        ecQpEstimatorPtr->getImpactModel("r_wrist")->getSurfaceNormal();
    double tempVel =
        surfaceNormal.transpose() *
        ecQpEstimatorPtr->getImpactModel("r_wrist")->getContactVel();
    // Eigen::Vector3d tempVel =
    // ecQpEstimatorPtr->getImpactModel("r_wrist")->getContactVel();

    Eigen::MatrixXd jacobian =
        ecQpEstimatorPtr->getImpactModel("r_wrist")->getJacobian();

    Eigen::Vector3d deltaV = jacobian * getOsd()->getInvMassMatrix() *
                             jacobian.transpose() *
  ...
  logger().addLogEntry("ZMP_World", [this]() {
    ...
    try {
      zmp =
          robot.zmp({"LeftFootForceSensor", "RightFootForceSensor"},
  ...
  gui()->addElement(
      ...
      mc_rtc::gui::Force(
          ...
          [this]() {
            ...
            const sva::PTransformd X_0_s =
                robot.surface("LeftFoot").X_0_s(robot);
      ...
      mc_rtc::gui::Force(
          ...
          [this]() {
            ...
            const sva::PTransformd X_0_s =
                robot.surface("RightFoot").X_0_s(robot);
            sva::PTransformd surface(X_0_s.rotation(), cop);
            return surface;
          }),
      mc_rtc::gui::Point3D("ZMP_real", [this]() {
        ...
        Eigen::Vector3d zmp;
        try {
          zmp = robot.zmp({"LeftFootForceSensor", "RightFootForceSensor"},
                          Eigen::Vector3d::Zero(), Eigen::Vector3d{0., 0., 1.},
                          100);
          zmp.z() = 0;
        } catch (...) {
          zmp.setZero();
  ...
  logger().addLogEntry("constraints_jointVelocity_on", [this]() -> double {
    return static_cast<bool>(
        config()("impact")("constraints")("jointVelocity")("on"));
  });
  logger().addLogEntry(
      "constraints_jointVelocity_multiplier", [this]() -> double {
        return config()("impact")("constraints")("jointVelocity")("multiplier");
      });
  logger().addLogEntry("constraints_jointTorque_on", [this]() -> double {
    return static_cast<bool>(
        config()("impact")("constraints")("jointTorque")("on"));
  });
  logger().addLogEntry(
      "constraints_jointTorque_multiplier", [this]() -> double {
        return config()("impact")("constraints")("jointTorque")("multiplier");
      });
  logger().addLogEntry(
      "constraints_frictionWithImpulse_on", [this]() -> double {
        return static_cast<bool>(
            config()("impact")("constraints")("frictionWithImpulse"));
      });
  logger().addLogEntry("constraints_copWithImpulse_on", [this]() -> double {
    return static_cast<bool>(
        config()("impact")("constraints")("copWithImpulse")("on"));
  });
  logger().addLogEntry("constraints_zmpWithImpulse_on", [this]() -> double {
    return static_cast<bool>(
        config()("impact")("constraints")("zmpWithImpulse")("on"));
  });
  logger().addLogEntry(
      "constraints_zmpWithImpulse_allforce", [this]() -> double {
        return static_cast<bool>(
            config()("impact")("constraints")("zmpWithImpulse")("allforce"));
      });

  std::string impactBodyString(
      config()("impact")("estimation")("impactBodyName"));

  miOsdPtr_ = std::make_shared<mc_impact::mi_osd>(robot(), true);
  std::vector<std::string> eeNameVector =
      config()("impact")("estimation")("end-effectors");

  miOsdPtr_->initializeDataStructure(static_cast<int>(eeNameVector.size()));
  miOsdPtr_->resetDataStructure();

  for (auto index = eeNameVector.begin(); index != eeNameVector.end();
       ++index) {
    ...
    if (!miOsdPtr_->addEndeffector(*index)) {
      ...
      std::cout << "End-effector: " << *index << "is added to the OSD model. "
                << std::endl;
    }
  }

  std::vector<std::string> contactBodies =
      config()("impact")("estimation")("contactBodies");
  miOsdPtr_->setContact(contactBodies);

  std::cout << "Operational space dynamics Predictor is created." << std::endl;

  if (config()("lcp")("on")) {
    lcpSolverPtr.reset(new mc_impact::mi_lcp(
        robot(), realRobots().robot(), getOsd(), config()("lcp")("dim"),
  ...
  if (config()("qpEstimator")("on")) {
    ...
    qpParams.coeFrictionDeduction =
        config()("impact")("estimation")("coeFrictionDeduction");
    ...
    std::string impactBodyName =
        config()("impact")("estimation")("impactBodyName");
    Eigen::Vector3d inertial_normal =
        config()("impact")("estimation")("inertialFrame_impactNormal");
    qpParams.impactNameAndNormals.insert(
        std::pair<std::string, Eigen::Vector3d>(impactBodyName,
                                                inertial_normal));
    qpParams.useLagrangeMultiplier = false;
    qpParams.useJsd = true;
    qpParams.useOsd = true;

    qpEstimatorPtr.reset(
        new mc_impact::mi_qpEstimator(robot(), getOsd(), qpParams));

    qpParams.useJsd = true;
    qpParams.useOsd = false;
    jsdQpEstimatorPtr.reset(
        new mc_impact::mi_qpEstimator(robot(), getOsd(), qpParams));

    qpParams.useJsd = false;
    qpParams.useOsd = true;
    osdQpEstimatorPtr.reset(
        new mc_impact::mi_qpEstimator(robot(), getOsd(), qpParams));

    qpParams.useJsd = false;
    qpParams.useOsd = true;
    qpParams.useLagrangeMultiplier = true;
    ecQpEstimatorPtr.reset(
        new mc_impact::mi_qpEstimator(robot(), getOsd(), qpParams));
    ...
    */
    ...
    std::cout << "QP impulse estimator has added the endeffectors."
              << std::endl;
  }
  ...
  logger().addLogEntry("ee_Vel_real", [this]() {
    return realRobots()
        .robot()
        .mbc()
        .bodyVelB[realRobots().robot().mb().bodyIndexByName("r_wrist")];
  });
  logger().addLogEntry("ee_Vel_robot", [this]() {
  ...
  logger().addLogEntry("l_ankle_Vel_robot", [this]() {
    return robot()
        .mbc()
        .bodyVelB[robot().mb().bodyIndexByName("l_ankle")]
        .linear();
  });
  ...
  logger().addLogEntry("l_ankle_Vel_real", [this]() {
    return realRobots()
        .robot()
        .mbc()
        .bodyVelB[realRobots().robot().mb().bodyIndexByName("l_ankle")]
        .linear();
  });

  logger().addLogEntry("r_ankle_Vel_robot", [this]() {
    return robot()
        .mbc()
        .bodyVelB[robot().mb().bodyIndexByName("r_ankle")]
        .linear();
  });
  ...
  logger().addLogEntry("r_ankle_Vel_real", [this]() {
    return realRobots()
        .robot()
        .mbc()
        .bodyVelB[realRobots().robot().mb().bodyIndexByName("r_ankle")]
        .linear();
  });
  logger().addLogEntry("COM_RightFootForceSensor", [this]() {
    ...
    sva::ForceVecd f_ee =
        X_ee_CoM.dualMul(robot().forceSensor("RightFootForceSensor").wrench());
    return f_ee;
  });
  ...
  logger().addLogEntry("COM_LeftFootForceSensor", [this]() {
    ...
    sva::PTransformd X_ee_CoM = X_0_CoM * X_0_ee.inv();
    sva::ForceVecd f_ee =
        X_ee_CoM.dualMul(robot().forceSensor("LeftFootForceSensor").wrench());
    return f_ee;
  });
  ...
  logger().addLogEntry("COM_RightHandForceSensor", [this]() {
    ...
    sva::PTransformd X_ee_CoM = X_0_CoM * X_0_ee.inv();
    sva::ForceVecd f_ee =
        X_ee_CoM.dualMul(robot().forceSensor("RightHandForceSensor").wrench());
    return f_ee;
  });
  ...
  logger().addLogEntry("COM_sum_sensor_wrench", [this]() {
    ...
    sva::ForceVecd f_rSole = X_rSole_CoM.dualMul(
    ...
    sva::ForceVecd f_lSole = X_lSole_CoM.dualMul(
  ...
  logger().addLogEntry("q_position", [this]() {
    return rbd::paramToVector(realRobots().robot().mb(),
                              realRobots().robot().mbc().q);
  });

  logger().addLogEntry("q_vel", [this]() {
    return rbd::dofToVector(realRobots().robot().mb(),
                            realRobots().robot().mbc().alpha);
  });

  logger().addLogEntry("q_acc", [this]() {
    return rbd::dofToVector(realRobots().robot().mb(),
                            realRobots().robot().mbc().alphaD);
  });
  ...
  logger().addLogEntry("impulsive_tau_righthand", [this]() {
    return (Eigen::VectorXd)(
        ecQpEstimatorPtr->getImpactModel("r_wrist")->getJacobian().transpose() *
  ...
  if (config()("qpEstimator")("on")) {
    ...
    logger().addLogEntry("qpOsd_RightFootForce", [this]() {
      return osdQpEstimatorPtr->getEndeffector("r_ankle")
          .estimatedAverageImpulsiveForce;
    });
    logger().addLogEntry("qpOsd_LeftFootForce", [this]() {
      return osdQpEstimatorPtr->getEndeffector("l_ankle")
          .estimatedAverageImpulsiveForce;
    });
    logger().addLogEntry("qpOsd_RightHandForce", [this]() {
      return osdQpEstimatorPtr->getEndeffector("r_wrist")
          .estimatedAverageImpulsiveForce;
    });
    logger().addLogEntry("qpOsd_delta_q_dot", [this]() {
      return osdQpEstimatorPtr->getJointVelJump();
    });
    logger().addLogEntry("qpOsd_delta_tau",
                         [this]() { return osdQpEstimatorPtr->getTauJump(); });

    logger().addLogEntry("qpOsd_delta_v_leftFoot", [this]() {
      return osdQpEstimatorPtr->getEndeffector("l_ankle").eeVJump;
    });

    logger().addLogEntry("qpOsd_delta_v_rightFoot", [this]() {
      return osdQpEstimatorPtr->getEndeffector("r_ankle").eeVJump;
    });
    logger().addLogEntry("qpOsd_delta_v_rightHand", [this]() {
      return osdQpEstimatorPtr->getEndeffector("r_wrist").eeVJump;
    });

    logger().addLogEntry("qpJsd_RightFootForce", [this]() {
      return jsdQpEstimatorPtr->getEndeffector("r_ankle")
          .estimatedAverageImpulsiveForce;
    });
    logger().addLogEntry("qpJsd_LeftFootForce", [this]() {
      return jsdQpEstimatorPtr->getEndeffector("l_ankle")
          .estimatedAverageImpulsiveForce;
    });
    logger().addLogEntry("qpJsd_RightHandForce", [this]() {
      return jsdQpEstimatorPtr->getEndeffector("r_wrist")
          .estimatedAverageImpulsiveForce;
    });
    logger().addLogEntry("qpJsd_delta_q_dot", [this]() {
      return jsdQpEstimatorPtr->getJointVelJump();
    });
    logger().addLogEntry("qpJsd_delta_tau",
                         [this]() { return jsdQpEstimatorPtr->getTauJump(); });

    logger().addLogEntry("qpJsd_delta_v_leftFoot", [this]() {
      return jsdQpEstimatorPtr->getEndeffector("l_ankle").eeVJump;
    });

    logger().addLogEntry("qpJsd_delta_v_rightFoot", [this]() {
      return jsdQpEstimatorPtr->getEndeffector("r_ankle").eeVJump;
    });
    logger().addLogEntry("qpJsd_delta_v_rightHand", [this]() {
      return jsdQpEstimatorPtr->getEndeffector("r_wrist").eeVJump;
    });
    logger().addLogEntry("qpEc_check_RightFootForce", [this]() {
      return ecQpEstimatorPtr->getEndeffector("r_ankle").checkForce;
    });
    logger().addLogEntry("qpEc_check_LeftFootForce", [this]() {
      return ecQpEstimatorPtr->getEndeffector("l_ankle").checkForce;
    });
    logger().addLogEntry("qpEc_check_RightHandForce", [this]() {
      return ecQpEstimatorPtr->getEndeffector("r_wrist").checkForce;
    });

    logger().addLogEntry("qpEc_RightFootForce", [this]() {
      return ecQpEstimatorPtr->getEndeffector("r_ankle")
          .estimatedAverageImpulsiveForce;
    });
    logger().addLogEntry("qpEc_LeftFootForce", [this]() {
      return ecQpEstimatorPtr->getEndeffector("l_ankle")
          .estimatedAverageImpulsiveForce;
    });
    logger().addLogEntry("qpEc_RightHandForce", [this]() {
      return ecQpEstimatorPtr->getEndeffector("r_wrist")
          .estimatedAverageImpulsiveForce;
    });
    logger().addLogEntry("qp_RightHand_impact_normal", [this]() {
      return ecQpEstimatorPtr->getImpactModel("r_wrist")->getSurfaceNormal();
    });

    logger().addLogEntry("qpEc_delta_q_dot", [this]() {
      return ecQpEstimatorPtr->getJointVelJump();
    });
    logger().addLogEntry("qpEc_delta_tau",
                         [this]() { return ecQpEstimatorPtr->getTauJump(); });

    logger().addLogEntry("qpEc_delta_v_leftFoot", [this]() {
      return ecQpEstimatorPtr->getEndeffector("l_ankle").eeVJump;
    });

    logger().addLogEntry("qpEc_delta_v_rightFoot", [this]() {
      return ecQpEstimatorPtr->getEndeffector("r_ankle").eeVJump;
    });
    logger().addLogEntry("qpEc_delta_v_rightHand", [this]() {
      return ecQpEstimatorPtr->getEndeffector("r_wrist").eeVJump;
    });
    logger().addLogEntry("qpEc_delta_v_impact", [this]() {
      return ecQpEstimatorPtr->getImpactModel("r_wrist")->getEeVelocityJump();
    });

    logger().addLogEntry("qpEc_ee_v_predict", [this]() {
      return ecQpEstimatorPtr->getImpactModel("r_wrist")->getEeVelocity();
    });

    logger().addLogEntry("qpEc_ee_v_normal", [this]() {
      return ecQpEstimatorPtr->getImpactModel("r_wrist")->getSurfaceNormal();
    });
    ...
    logger().addLogEntry("qpEc_compare_delta_dq", [this]() {
      ...
      return (ecQpEstimatorPtr->getJointVelJump() -
              ecQpEstimatorPtr->getJacobianDeltaAlpha() *
    ...
    logger().addLogEntry("qpEc_compare_RightFootForce", [this]() {
      ...
      Eigen::Vector3d predictedForce =
          inv_dt * (ecQpEstimatorPtr->getJacobianDeltaF("r_ankle") * temp);
      // Eigen::Vector3d predictedForce =
    ...
    logger().addLogEntry("qpEc_compare_LeftFootForce", [this]() {
      ...
      double inv_dt =
          ...
          ecQpEstimatorPtr->getImpactModel("r_wrist")->getImpactDuration();
      Eigen::Vector3d predictedForce =
          inv_dt * (ecQpEstimatorPtr->getJacobianDeltaF("l_ankle") * temp);
      // Eigen::Vector3d predictedForce =
    ...
    logger().addLogEntry("qpEc_compare_RightHandForce", [this]() {
      ...
      Eigen::Vector3d predictedForce =
          inv_dt * (ecQpEstimatorPtr->getJacobianDeltaF("r_wrist")) *
          ecQpEstimatorPtr->getImpactModel("r_wrist")->getJointVel();
      return predictedForce;
    });
    ...
    logger().addLogEntry("qpEc_compare_delta_v", [this]() {
      Eigen::VectorXd temp =
          ...
          rbd::dofToVector(robot().mb(), robot().mbc().alphaD) * timeStep;
      Eigen::Vector3d diff =
          ecQpEstimatorPtr->getImpactModel("r_wrist")->getEeVelocityJump() -
    ...
    logger().addLogEntry("qp_RightFootForce", [this]() {
      return qpEstimatorPtr->getEndeffector("r_ankle")
          .estimatedAverageImpulsiveForce;
    });
    logger().addLogEntry("qp_LeftFootForce", [this]() {
      return qpEstimatorPtr->getEndeffector("l_ankle")
          .estimatedAverageImpulsiveForce;
    });
    logger().addLogEntry("qp_RightHandForce", [this]() {
      return qpEstimatorPtr->getEndeffector("r_wrist")
          .estimatedAverageImpulsiveForce;
    });

    logger().addLogEntry("qp_delta_q_dot", [this]() {
      return qpEstimatorPtr->getJointVelJump();
    });
    logger().addLogEntry("qp_delta_tau",
                         [this]() { return qpEstimatorPtr->getTauJump(); });

    logger().addLogEntry("qp_delta_v_leftFoot", [this]() {
      return qpEstimatorPtr->getEndeffector("l_ankle").eeVJump;
    });

    logger().addLogEntry("qp_delta_v_rightFoot", [this]() {
      return qpEstimatorPtr->getEndeffector("r_ankle").eeVJump;
    });

    logger().addLogEntry("qp_delta_v_rightHand", [this]() {
      return qpEstimatorPtr->getEndeffector("r_wrist").eeVJump;
    });
    logger().addLogEntry("qp_delta_v_impact", [this]() {
      return qpEstimatorPtr->getImpactModel("r_wrist")->getEeVelocityJump();
    });
  }
  ...
  if (config()("lcp")("on")) {
    logger().addLogEntry("lcp_RightFootForce", [this]() {
      return lcpSolverPtr->getPredictedContactForce("r_ankle");
    });
    logger().addLogEntry("lcp_LeftFootForce", [this]() {
      return lcpSolverPtr->getPredictedContactForce("l_ankle");
    });
  }
  ...
  logger().addLogEntry("ZMP_QP", [this]() {
    ...
    sva::ForceVecd F_r_qp = X_rSole_0.dualMul(
        solver().desiredContactForce(getContact("RightFoot")));
    sva::ForceVecd F_l_qp =
        X_lSole_0.dualMul(solver().desiredContactForce(getContact("LeftFoot")));
  ...
  logger().addLogEntry("ZMP_test_calculation_allforce", [this]() {
    ...
    double denominator =
        fr_0.force().z() + fl_0.force().z() + free_0.force().z();

    Eigen::Vector3d tempZMP;
    tempZMP.x() =
        -(fr_0.couple().y() + fl_0.couple().y() + free_0.couple().y()) /
        denominator;
    tempZMP.y() =
        (fr_0.couple().x() + fl_0.couple().x() + free_0.couple().x()) /
        denominator;
    tempZMP.z() = 0;
    return tempZMP;
  });
...
bool Controller::rArmInContact() {
  ...
  if (robot().forceSensor("RightHandForceSensor").force().norm() >
      forceThreshold) {
    impactIndicator_ = 20;
    return true;
  } else
...
bool Controller::run() {
  ...
  if (iter_++ == 0) {
    ...
    if (config()("impact")("constraints")("zmpWithImpulse")("on")) {
      ...
      std::vector<double> zmpVector =
          config()("impact")("constraints")("zmpWithImpulse")("supportPolygon");
      ...
      zmpImpulse_.reset(new mc_impact::zmpWithImpulse(
          *ecQpEstimatorPtr, supports, timeStep,
          ecQpEstimatorPtr->getImpactModel("r_wrist")->getImpactDuration(),
          {zmpVector[0], zmpVector[1], zmpVector[2],
           zmpVector[3]}, /// This is the supportPolygon
          allforce, debug));

      solver().addConstraint(zmpImpulse_.get());

      logger().addLogEntry("ZMP_x_lower_bound", [this]() {
        return (double)zmpImpulse_->area_.min_x;
      });

      logger().addLogEntry("ZMP_x_upper_bound", [this]() {
        return (double)zmpImpulse_->area_.max_x;
      });

      logger().addLogEntry("ZMP_y_lower_bound", [this]() {
        return (double)zmpImpulse_->area_.min_y;
      });

      logger().addLogEntry("ZMP_y_upper_bound", [this]() {
        return (double)zmpImpulse_->area_.max_y;
      });
    }
    ...
    if (config()("impact")("constraints")("copWithImpulse")("on")) {
      ...
      std::vector<double> contactArea =
          config()("impact")("constraints")("copWithImpulse")("contactArea");
      copImpulseRightFoot_.reset(new mc_impact::copWithImpulse(
          *ecQpEstimatorPtr, "r_ankle", "RightFootForceSensor", timeStep,
          ecQpEstimatorPtr->getImpactModel("r_wrist")->getImpactDuration(),
          {contactArea[0], contactArea[1], contactArea[2], contactArea[3]}));
      solver().addConstraint(copImpulseRightFoot_.get());

      copImpulseLeftFoot_.reset(new mc_impact::copWithImpulse(
          *ecQpEstimatorPtr, "l_ankle", "LeftFootForceSensor", timeStep,
          ecQpEstimatorPtr->getImpactModel("r_wrist")->getImpactDuration(),
          {contactArea[0], contactArea[1], contactArea[2], contactArea[3]}));
      ...
      logger().addLogEntry("CoP_LeftFoot_local_constraint",
                           [this]() { return copImpulseLeftFoot_->getCop(); });

      logger().addLogEntry("CoP_RightFoot_local_constraint",
                           [this]() { return copImpulseRightFoot_->getCop(); });

      logger().addLogEntry("CoP_LeftFoot_local_constraint_allforce", [this]() {
        return copImpulseLeftFoot_->getCopPerturbWhole();
      });

      logger().addLogEntry("CoP_RightFoot_local_constraint_allforce", [this]() {
        return copImpulseRightFoot_->getCopPerturbWhole();
      });

      logger().addLogEntry("CoP_x_lower_bound", [this]() {
        return (double)copImpulseLeftFoot_->area_.min_x;
      });

      logger().addLogEntry("CoP_x_upper_bound", [this]() {
        return (double)copImpulseLeftFoot_->area_.max_x;
      });

      logger().addLogEntry("CoP_y_lower_bound", [this]() {
        return (double)copImpulseLeftFoot_->area_.min_y;
      });

      logger().addLogEntry("CoP_y_upper_bound", [this]() {
        return (double)copImpulseLeftFoot_->area_.max_y;
      });
    }
    if (config()("impact")("constraints")("frictionWithImpulse")) {
      frictionImpulseLeftFoot_.reset(new mc_impact::frictionWithImpulse(
          *ecQpEstimatorPtr, "l_ankle", "LeftFootForceSensor",
          getContact("LeftFoot"), timeStep,
          ecQpEstimatorPtr->getImpactModel("r_wrist")->getImpactDuration()));
      solver().addConstraint(frictionImpulseLeftFoot_.get());

      frictionImpulseRightFoot_.reset(new mc_impact::frictionWithImpulse(
          *ecQpEstimatorPtr, "r_ankle", "RightFootForceSensor",
          getContact("RightFoot"), timeStep,
          ecQpEstimatorPtr->getImpactModel("r_wrist")->getImpactDuration()));
    ...
    if (config()("impact")("constraints")("jointTorque")("on")) {
      ...
      boundTorqueJump_.reset(new mc_impact::BoundJointTorqueJump(
          *ecQpEstimatorPtr, timeStep,
          ecQpEstimatorPtr->getImpactModel("r_wrist")->getImpactDuration(),
          config()("impact")("constraints")("jointTorque")("multiplier"),
          debugTorque));
      ...
      logger().addLogEntry("qpEc_delta_tau_upper_bound", [this]() {
        double multiplier =
            config()("impact")("constraints")("jointTorque")("multiplier");
        return (Eigen::VectorXd)(multiplier *
                                 rbd::dofToVector(robot().mb(), robot().tu()));
      });
      ...
      logger().addLogEntry("qpEc_delta_tau_lower_bound", [this]() {
        double multiplier =
            config()("impact")("constraints")("jointTorque")("multiplier");
        return (Eigen::VectorXd)(multiplier *
                                 rbd::dofToVector(robot().mb(), robot().tl()));
      });
    }

    if (config()("impact")("constraints")("jointVelocity")("on")) {
      bool debugVelocity =
          config()("impact")("constraints")("jointVelocity")("debug");
      double deductFactor =
          config()("impact")("constraints")("jointVelocity")("multiplier");

      boundVelocityJump_.reset(new mc_impact::BoundJointVelocityJump(
          *ecQpEstimatorPtr, timeStep, deductFactor, debugVelocity));
      ...
      logger().addLogEntry("qpEc_delta_q_dot_lower_bound", [this]() {
        double multiplier =
            config()("impact")("constraints")("jointVelocity")("multiplier");
        return (Eigen::VectorXd)(multiplier *
                                 rbd::dofToVector(robot().mb(), robot().vl()));
      });
      ...
      logger().addLogEntry("qpEc_delta_q_dot_upper_bound", [this]() {
        double multiplier =
            config()("impact")("constraints")("jointVelocity")("multiplier");
        return (Eigen::VectorXd)(multiplier *
                                 rbd::dofToVector(robot().mb(), robot().vu()));
      });
    }
