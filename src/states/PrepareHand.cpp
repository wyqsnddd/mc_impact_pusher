...
void PrepareHandState::start(mc_control::fsm::Controller & ctlInput)
{
  ...
  if(ctl.config()("impact")("constraints")("zmpWithImpulse")("on")){
    ...
    int nDof = ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getRobot().mb().nrDof();
    A_.resize(4, nDof);
    A_.setZero();
    b_.setZero();
    A_ = (ctl.timeStep/ ctl.timeStep) * ctl.zmpImpulse_->getZMP()* sumJac;
  // std::cout<<"size of A_: "<<A_.rows()<<", "<<A_.cols()<<std::endl;
    rbd::paramToVector(ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getRobot().mbc().alpha, alpha_);
...
/*
  ...
  ctl.logger().addLogEntry("l_ankle_CoP_Constraint", [&ctl]() {
    ...
    sva::ForceVecd r_impulse =
        X_rSole_lSole.dualMul(sva::ForceVecd(Eigen::Vector3d::Zero(), ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("r_sole")));
    sva::ForceVecd l_impulse = sva::ForceVecd(Eigen::Vector3d::Zero(), ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("l_sole"));

    ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("l_sole");
    sva::PTransformd X_ee_lSole = X_0_lSole * X_0_ee.inv();
    sva::ForceVecd f_ee =
        X_ee_lSole.dualMul(sva::ForceVecd(Eigen::Vector3d::Zero(), ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce()));
  ...
  ctl.logger().addLogEntry("r_ankle_CoP_Constraint", [&ctl]() {
    ...
    sva::ForceVecd l_impulse =
        X_lSole_rSole.dualMul(sva::ForceVecd(Eigen::Vector3d::Zero(), ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("l_sole")));

    sva::ForceVecd r_impulse = sva::ForceVecd(Eigen::Vector3d::Zero(), ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("r_sole"));

    sva::PTransformd X_ee_rSole = X_0_rSole * X_0_ee.inv();
    sva::ForceVecd f_ee =
        X_ee_rSole.dualMul(sva::ForceVecd(Eigen::Vector3d::Zero(), ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce()));
  ...
  ctl.logger().addLogEntry("l_ankle_zero_slippage", [&ctl]() {
    ...
    Eigen::Vector3d f_qp = (ctl.robot().forceSensor("LeftFootForceSensor").force());
    // Eigen::Vector3d result = multiplier * (f_qp + ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("l_sole"));
    ...
    Eigen::Vector2d result = multiplier_ * (f_qp - ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("l_sole"));
    return result;
  });
  ...
  ctl.logger().addLogEntry("r_ankle_zero_slippage", [&ctl]() {
    ...
    Eigen::Vector3d f_qp = (ctl.robot().forceSensor("RightFootForceSensor").force());
    // Eigen::Vector3d result = multiplier * (f_qp + ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("r_sole"));
    Eigen::MatrixXd multiplier_;
    multiplier_.resize(2, 3);
    multiplier_.setZero();
    multiplier_(0, 0) = 1.0;
    ...
    Eigen::Vector2d result = multiplier_ * (f_qp - ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("r_sole"));
    return result;
  });
}
...
bool PrepareHandState::run(mc_control::fsm::Controller & ctlInput)
{
  ...
  sva::PTransformd X_0_ee = ctl.robot().bodyPosW("r_wrist");
  // ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->run(surfaceNormal);
...
void PrepareHandState::teardown(mc_control::fsm::Controller & ctl_)
{
  ...
  // ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->resetDataStructure();
}
