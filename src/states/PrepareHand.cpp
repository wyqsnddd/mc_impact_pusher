...
void PrepareHandState::start(mc_control::fsm::Controller & ctlInput)
{
   ...
   // std::cout<<"size of alpha_"<<alpha_.rows()<<std::endl;
     ...
     */
      /*
         ...
         ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->());
  ...
  ctl.logger().addLogEntry("l_ankle_CoP_Constraint", [&ctl]() {
    ...
    sva::ForceVecd r_impulse = X_rSole_lSole.dualMul(sva::ForceVecd(
        Eigen::Vector3d::Zero(), ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("r_sole")));
    sva::ForceVecd l_impulse = sva::ForceVecd(
        Eigen::Vector3d::Zero(), ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("l_sole"));

    ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("l_sole");
    sva::PTransformd X_ee_lSole = X_0_lSole * X_0_ee.inv();
    sva::ForceVecd f_ee = X_ee_lSole.dualMul(sva::ForceVecd(
        Eigen::Vector3d::Zero(), ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce()));
  ...
  ctl.logger().addLogEntry("r_ankle_CoP_Constraint", [&ctl]() {
    ...
    sva::ForceVecd r_impulse = sva::ForceVecd(
        Eigen::Vector3d::Zero(), ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("r_sole"));

    sva::PTransformd X_ee_rSole = X_0_rSole * X_0_ee.inv();
    sva::ForceVecd f_ee = X_ee_rSole.dualMul(sva::ForceVecd(
        Eigen::Vector3d::Zero(), ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce()));
  ...
  ctl.logger().addLogEntry("l_ankle_zero_slippage", [&ctl]() {
    ...
    // ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("l_sole")); Eigen::Vector3d result =
    // multiplier * (f_qp); return result;
    Eigen::MatrixXd multiplier_;
    multiplier_.resize(2, 3);
    multiplier_.setZero();
    multiplier_(0, 0) = 1.0;
    ...
    Eigen::Vector2d result =
        multiplier_ * (f_qp - ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("l_sole"));
    return result;
  });
  ...
  ctl.logger().addLogEntry("r_ankle_zero_slippage", [&ctl]() {
    ...
    Eigen::Vector3d f_qp = (ctl.robot().forceSensor("RightFootForceSensor").force());
    // Eigen::Vector3d result = multiplier * (f_qp +
    // ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("r_sole"));
    Eigen::MatrixXd multiplier_;
    multiplier_.resize(2, 3);
    multiplier_.setZero();
    multiplier_(0, 0) = 1.0;
    ...
    Eigen::Vector2d result =
        multiplier_ * (f_qp - ctl.multiImpactPredictorPtr->getPredictor("r_wrist")->getImpulsiveForce("r_sole"));
    return result;
  });
}
