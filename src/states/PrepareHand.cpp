...
void PrepareHandState::start(mc_control::fsm::Controller &ctlInput) {
  ...
  std::vector<std::string> right_joints = {
      "CHEST_Y",   "R_SHOULDER_P", "R_SHOULDER_R", "R_SHOULDER_Y",
  ...
  ctl.comTaskPtr =
      mc_tasks::MetaTaskLoader::load(ctl.solver(), ctl.config()("com"));
  ...
  rEfTaskPtr_->set_ef_pose(sva::PTransformd(
      desiredRotation,
      rEfTaskPtr_->get_ef_pose().translation() + translation_offset));

  run(ctlInput);
  //--------------------------- Joint velocity and torque jump
  bool debugTorque = ctl.config()("impact")("constraints")("jointTorque")("on");
  bool debugVelocity =
      ctl.config()("impact")("constraints")("jointVelocity")("on");
  if (debugTorque) {
    ...
    ctl.logger().addLogEntry("qp_boundTau_delta_tau_compare", [&ctl]() {
      return (Eigen::VectorXd)(ctl.boundTorqueJump_->getDeltaTau() -
                               ctl.ecQpEstimatorPtr->getTauJump())
  ...
  if (debugVelocity) {
    ...
    ctl.logger().addLogEntry("qp_boundJointVel_delta_vel_compare", [&ctl]() {
      return (Eigen::VectorXd)(ctl.boundVelocityJump_->getDeltaVel() -
                               ctl.ecQpEstimatorPtr->getJointVelJump())
          .segment(6, ctl.ecQpEstimatorPtr->getDof() - 6);
    });
  }
  ...
  ctl.logger().addLogEntry("l_ankle_CoP_Constraint_sensor", [&ctl]() {
    ...
    // auto sensorForce =
    // ctl.solver().desiredContactForce(ctl.getContact("LeftFoot"));
  ...
  ctl.logger().addLogEntry("r_ankle_CoP_Constraint_sensor", [&ctl]() {
    ...
    // auto sensorForce =
    // ctl.solver().desiredContactForce(ctl.getContact("RightFoot"));
    Eigen::VectorXd result = A_cop * (sensorForce.vector());
    return result;
  });
  ...
  if (ctl.config()("impact")("constraints")("zmpWithImpulse")("on")) {
    ...
    ctl.logger().addLogEntry("ZMP_perturbation_pconstraint", [&ctl]() {
      return ctl.zmpImpulse_->getZMP_perturbation();
    });

    ctl.logger().addLogEntry("ZMP_sensor_constraint", [&ctl]() {
      return ctl.zmpImpulse_->getZMP_sensor();
    });

    ctl.logger().addLogEntry("ZMP_constraint_difference", [&ctl]() {
      Eigen::VectorXd difference =
          ctl.zmpImpulse_->getZMP_constraint_difference();
      return difference;
    });

    ctl.logger().addLogEntry("ZMP_prediction_allforce_constraint", [&ctl]() {
      return ctl.zmpImpulse_->getZMP_prediction_allforce();
    });
    ctl.logger().addLogEntry("ZMP_prediction_feetforce_constraint", [&ctl]() {
      return ctl.zmpImpulse_->getZMP_prediction_feetforce();
    });

    ctl.logger().addLogEntry("ZMP_Constraint_test", [&ctl]() {
      Eigen::VectorXd result =
          ctl.zmpImpulse_->getA() *
              rbd::dofToVector(ctl.robot().mb(), ctl.robot().mbc().alphaD) -
  ...
  ctl.logger().addLogEntry("r_ankle_tangential_force", [&ctl]() {
    ...
    Eigen::Matrix3d nullProjector =
        Eigen::MatrixXd::Identity(3, 3) - normal * normal.transpose();

    Eigen::Vector3d f_qp =
        (ctl.robot().forceSensor("RightFootForceSensor").force());
  ...
  ctl.logger().addLogEntry("l_ankle_tangential_force", [&ctl]() {
    ...
    Eigen::Vector3d f_qp =
        (ctl.robot().forceSensor("LeftFootForceSensor").force());
    return (nullProjector * f_qp).norm();
  });
...
bool PrepareHandState::run(mc_control::fsm::Controller &ctlInput) {
  ...
  auto &ctl = static_cast<Controller &>(ctlInput);
  // std::cout << "PrepareState: the right ef error is: " <<
  ...
  if (ctl.config()("lcp")("on")) {
    if (ctl.lcpSolverPtr->getDim() == 1) {
      ...
      contactSurfaceNormals["l_ankle"] =
          X_0_lSole.rotation() * groundSurfaceNormal + X_0_lSole.translation();
      sva::PTransformd X_0_rSole = ctl.robot().bodyPosW("r_ankle");
      contactSurfaceNormals["r_ankle"] =
          X_0_rSole.rotation() * groundSurfaceNormal + X_0_rSole.translation();
  ...
  if (rEfTaskPtr_->eval().norm() <= efThreshold_) {
    // Output the transition signal such that we can move on according to the
    // transitions
