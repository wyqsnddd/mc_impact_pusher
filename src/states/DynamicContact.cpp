...
void DynamicContactState::start(mc_control::fsm::Controller &ctlInput) {
  ...
  rPosTaskPtr_ = std::make_shared<mc_tasks::EndEffectorTask>(
      efName, ctl.robots(), 0, conf("rightEfTaskDynamic")("stiffness"),
  ...
  ctl.logger().addLogEntry("ee_Vel_target", [this]() {
    return rPosTaskPtr_->positionTask->refVel();
  });

  currentPos = ctl.robot()
                   .mbc()
                   .bodyPosW[ctl.robot().bodyIndexByName(efName)]
                   .translation();
  ...
  if (state_conf_.has("rightEfStiffness")) {
    rPosTaskPtr_->positionTask->stiffness(
        static_cast<double>(state_conf_("rightEfStiffness")));
  }
  if (state_conf_.has("rightEfDamping")) {
    rPosTaskPtr_->positionTask->damping(
        static_cast<double>(state_conf_("rightEfDamping")));
  }
